...
def load_images(fname):
    ...
    return np.array(
        [
            [
                images.subsurface((i * fsize, j * fsize, fsize, fsize))
...
class Player:
    ...
    def step(self, pressed_keys):
        ...
        pos_in_plan = (
            (self.posx + self.grid // 2) // self.grid,
            (self.posy + self.grid // 2) // self.grid,
        )
        ...
        killing_monster = len(
            [
                m
                for m in monsters
                if abs(self.posx * m.grid - m.posx * self.grid)
                + abs(self.posy * m.grid - m.posy * self.grid)
                < m.grid * self.grid // 2
            ]
        )

        if (plan[pos_in_plan] == fire_id or killing_monster) and (
            self.orientation != 4 or self.phase == 0
        ):  # Player is hit by fire or monster?
            self.orientation = 4
            self.phase = 0
            ...
            self.my_bombs = [
                bomb_pos for bomb_pos in self.my_bombs if plan[bomb_pos] == bomb_id
            ]
            if (
                len(self.my_bombs) < self.max_bombs
                and in_pressed_keys(self.kbomb)
                and not plan[pos_in_plan]
            ):  # Drop a bomb?
            ...
            if stepx or stepy:
                ...
                self.orientation = (
                    2 if stepx > 0 else 1 if stepx < 0 else (stepy < 0) * 3
                )
            else:
                self.phase = 0
            ...
            blocked_corners = (
                plan[
                    np.ix_(
                        (self.posx + stepx + self.field_span) // self.grid,
                        (self.posy + stepy + self.field_span) // self.grid,
                    )
                ]
                > item_id
            )
            corner_xing_field = np.logical_or.outer(
                np.logical_and(not self.posx % self.grid, [stepx < 0, stepx > 0]),
                np.logical_and(not self.posy % self.grid, [stepy < 0, stepy > 0]),
            )
            blocked_corners = np.logical_and(blocked_corners, corner_xing_field)
            if (
                stepx and stepy and blocked_corners.sum() == 1
            ):  # steping diagonally against corner
                if self.posx % self.grid or not (
                    self.posy % self.grid
                    or plan[self.posx // self.grid + stepx, self.posy // self.grid]
                ):
                    stepy = 0
                else:
                    stepx = 0
                ...
                stepx += np.sign(
                    np.sum(blocked_corners.T * [1, -1])
                )  # step back in the blocked directiones
                stepy += np.sign(np.sum(blocked_corners * [1, -1]))
            if (stepx or stepy) and not (
                blocked_corners.sum() == 2
                and np.all(blocked_corners == blocked_corners.T)
            ):
        ...
        screen.blit(
            self.sprites[self.orientation * 4 + self.phase // 2],
            (
                self.posx * fsize // self.grid,
...
class Monster:
    ...
    def step(self, pressed_keys):
        ...
        if plan[x, y] == fire_id:  # monster is hit by fire
            ...
            self.phase += 1
            if (
                self.phase == self.animation_frames + self.dying_animation_frames
            ):  # monster is completely dead
                monsters.remove(self)
                return
        else:
            target_field_blocked = (
                plan[
                    (self.posx + self.stepx + self.grid * (self.stepx == 1))
                    // self.grid,
                    (self.posy + self.stepy + self.grid * (self.stepy == 1))
                    // self.grid,
                ]
                >= wall_id + self.ghost
            )
            ...
            if (
                not field_centered and target_field_blocked
            ):  # Somebody just blocked the way with a bomb. Turn back!
                self.stepx = -self.stepx
                self.stepy = -self.stepy
            elif (
                self.stepx + self.stepy == 0
                or field_centered
                and (target_field_blocked or not np.random.randint(MONSTER_MIND))
            ):  # or we just randomly decided to turn
                possible_steps = np.nonzero(
                    plan[([x - 1, x + 1, x, x], [y, y, y - 1, y + 1])]
                    < wall_id + self.ghost
                )[0]
                newdir = (
                    np.random.choice(possible_steps) if len(possible_steps) else -1
...
while not game_over:
    ...
    plan_phase = np.zeros_like(
        plan, dtype=int
    )  # phase of fire, explosion, collapsing wall, bomb to explode
    ...
    wall_positiones = np.random.choice(
        np.where(wall_possible.flat)[0],
        min(wall_possible.sum(), WALL_NUMBER + TOOTHER_NUMBER + GHOST_NUMBER),
        replace=False,
    )
    monster_positiones = np.unravel_index(
        wall_positiones[: TOOTHER_NUMBER + GHOST_NUMBER], plan.shape
    )
    wall_positiones = wall_positiones[TOOTHER_NUMBER + GHOST_NUMBER :]
    plan.flat[wall_positiones] = wall_id
    plan_data.flat[
        wall_positiones[:BOMB_ITEM_NUMBER]
    ] = 1  # hide items under some of the walls
    ...
    while not game_over and len(players) > 1:
        ...
        for x, y in zip(*np.nonzero(np.logical_and(plan == bomb_id, plan_phase == 0))):
            ...
            h_fire_range = range(
                max(x - bomb_range, blocking_fire[blocking_fire < x][-1] + 1),
            ...
            v_fire_range = range(
                max(y - bomb_range, blocking_fire[blocking_fire < y][-1] + 1),
        ...
        for x, row in enumerate(plan):
            for y, field in enumerate(row):
                ...
                if field == rock_id:
                    ...
                    tile = i_bombs[
                        (BOMB_TIMER - plan_phase[x, y]) * len(i_bombs) // BOMB_TIMER
                    ]
                    plan_phase[x, y] -= 1
                elif field == fire_id:
                    plan_phase[x, y] += 1
                    if plan_phase[x, y] < fire_frames:  # explosion is not over
                        tile = i_fires[
                            plan_data[x, y],
                            plan_phase[x, y] * i_fires.shape[1] // fire_frames,
                        ]
                    else:
                        plan[x, y] = floor_id
                        # tile = i_floor
                    ...
                    if plan_phase[x, y] == 0:  # wall is intact
                        ...
                        plan_phase[x, y] == wall_frames
                    ):  # wall has just completely collapsed
                        plan[x, y] = (
                            item_id if plan_data[x, y] != 0 else floor_id
                        )  # Was the wall hidding an item?
                        plan_phase[x, y] = 0
                ...
                if plan[x, y] == item_id:
                    if (
                        plan_phase[x, y] == explosion_frames
                    ):  # item has just completely burned
                        plan[x, y] = floor_id
                        # tile = i_floor
                    else:
                        tile = i_abomb if plan_data[x, y] == 1 else i_afire
                        if plan_phase[x, y] > 0:  # item is burning
                            screen.blit(
                                tile
                                if plan_phase[x, y] < explosion_frames * 2 // 3
                                else i_floor,
                                (x * fsize, y * fsize - 16),
                            )
                            tile = i_explosion[
                                plan_phase[x, y] * len(i_explosion) // explosion_frames
                            ]
                            plan_phase[x, y] += 1
                screen.blit(tile, (x * fsize, y * fsize - 16))
        if len(players) < 2:
